  ...
  return (
    <div style={{ width: '100%' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '2rem' }} className="animate-fade-in">
        <div>
          ...
          <p style={{ color: 'var(--color-text-secondary)', marginTop: '0.25rem' }}>
            Tous les documents numérisés et envoyés sur la plateforme
          </p>
        </div>
        <span className="badge badge-primary" style={{ fontSize: '0.85rem', padding: '6px 14px' }}>
          {fichiers.length} documents
        </span>
      </div>
      ...
      <div className="input-group animate-fade-in-up" style={{ maxWidth: 420, marginBottom: '1.5rem' }}>
        <input
          className="input"
          placeholder="Rechercher un fichier ou utilisateur..."
          value={search}
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={5} style={{ textAlign: 'center', padding: '3rem', color: 'var(--color-text-muted)' }}>Aucun document trouvé</td></tr>
            ) : filtered.map(f => (
